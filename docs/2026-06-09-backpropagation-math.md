...
前向传播用 Jacobian $A$ 把输入位移 $dx$ 映射成输出位移 $dy$；反向传播用 $A^T$ 把输出侧梯度拉回输入侧梯度。
...
$dL$ 是微分，本质上是一个线性函数。它接收一个微小位移 $dX$，输出标量变化 $dL$。

这里的 $\langle\cdot,\cdot\rangle$ 是所在空间的内积。对于不同空间：
