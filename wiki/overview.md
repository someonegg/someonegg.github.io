...
- 当前顶层分类轴已稳定为 8 条：推理阶段控制、Agent 运行时可靠性原语、Agent 记忆机制、学习与思维方法、媒介理论与 AI、训练稳定性与优化、后训练策略学习、Prompt Optimization。
- “训练稳定性与优化”分类轴用于组织 `mHC`、`Muon`、`ERC` 等训练内稳定化机制，并与推理阶段控制分层。
- `Prompt Optimization` 作为独立方法学分类轴，覆盖从离散搜索到文本梯度，再到合成反馈驱动的闭环优化。
- 后训练策略学习作为独立分类轴，当前主线可概括为：`OPD` 用 `on-policy` 采样结合 `dense` 教师监督，`PPO` 仍是关键基线，`DPO/GRPO` 分别沿“偏好对齐流程简化”和“推理任务资源效率”重写该基线。
...
- 当前 `overview` 采用“两层口径”整理全局结构：
  - 顶层分类轴：推理阶段控制、Agent 运行时可靠性原语、Agent 记忆机制、学习与思维方法、媒介理论与 AI、训练稳定性与优化、后训练策略学习、Prompt Optimization。
  - 跨轴主题线索：长上下文注意力工程、`KV cache` serving、`DeepSeek-V4` 基建协同、`Reasoning Shift`、`ReasoningBank/MaTTS`。
